...
import "../contracts/interfaces/IVerifyBLS.sol";
import "./verifierActions.sol";

contract ReceiverOnNicoLayerOne is ITeleporterReceiver {
    // set address of ICM deployed contract on my chain
    ITeleporterMessenger public immutable messenger = ITeleporterMessenger(0x9bC0d2d756E9113523135E196DA4AF265D3302B0);
        ...
        external
    {
        ...
        if (actionType == VerifierAction.singleVerify) {
            (string memory messageValue, bytes memory signature, bytes memory publicKey) =
                abi.decode(paramsData, (string, bytes, bytes));
            ...
            (string memory messageValue, bytes[] memory signatures, bytes[] memory publicKeys) =
                abi.decode(paramsData, (string, bytes[], bytes[]));
            ...
            bytes memory aggSignature = verifyBLSPrecompile.aggregateSignatures(signatures);
            ...
            // hardcoding value since precompile is not deployed
            result = VerificationResult(false, verified);
        } else {
